...
# ./run-initial-config.sh --cluster-name=eks-dco-kx1z-cluster --region=us-east-1 --profile=tfsawsdne01-cloud-admin
...
echo -e "\n"
echo "Modify :: Adding EBS Driver and EBS Storage class"
echo  "Please refer to the https://github.tfs.toyota.com/dne-cloud/aws-ebs-csi-driver/blob/master/EBS-CSI-Driver-TMCC.md in case of an issue"
git clone git@github.tfs.toyota.com:dne-cloud/aws-ebs-csi-driver.git
cd aws-ebs-csi-driver-master/deploy/kubernetes/overlays/stable/ecr
kubectl apply -k .
cd -
...
sed -i -e "s|{{EBS_KMS_KEY_ARN}}|${ebs_key_arn}|g" storageclass/storage-class-ebs-gp3.yaml
kubectl apply -f storageclass/storage-class-ebs-gp3.yaml
...
cd aws-efs-csi-driver-master/deploy/kubernetes/overlays/stable/ecr
kubectl apply -k .
cd -
...
echo "Modify :: Applying tfsekstffulladmincluster-role.yaml"
sed -i -e "s|{{accountalias}}|${accountalias}|g" rbac/tfsekstffulladmincluster-role.yaml
kubectl apply -f rbac/tfsekstffulladmincluster-role.yaml

echo "Modify :: Applying tffulladmincluster-rbac.yaml"
sed -i -e "s|{{accountalias}}|${accountalias}|g" rbac/tffulladmincluster-rbac.yaml
kubectl apply -f rbac/tffulladmincluster-rbac.yaml

echo "Modify :: Applying servicenow-read-only-clusterrole.yaml"
kubectl apply -f rbac/servicenow-read-only-clusterrole.yaml

echo "Modify :: Applying servicenow-read-only-clusterrolebind.yaml"
kubectl apply -f rbac/servicenow-read-only-clusterrolebind.yaml
...
# implementing Ambassador CRD for ingress controller

mkdir -p ambcrd
cp ../../../standard-eks-config-files/crd/* ambcrd/
echo "Applying ambassador CRDs"
kubectl apply -f ambcrd/ambassador-crds.yaml
