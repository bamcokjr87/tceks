locals {
  worker-node-userdata-blue-spot = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_template" "launch_template-blue" {

  name     = "${aws_eks_cluster.cluster.name}-launch_template-blue"
  ...
  user_data              = base64encode(local.worker-node-userdata-blue-spot)
  update_default_version = true

}

resource "aws_autoscaling_group" "workers_asg_blue" {
  count            = var.activate-blue-asg-spot == "true" ? 1 : 0
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.launch_template-blue.id
      }
      dynamic "override" {
        iterator = overrides
        for_each = var.spot_instance_overrides 
        content{
