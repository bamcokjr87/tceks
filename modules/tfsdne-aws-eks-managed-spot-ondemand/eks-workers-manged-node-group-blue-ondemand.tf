locals {
  worker-managed-node-userdata-blue-ondemand = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_template" "managed_launch_template-blue-ondemand"{
    
    name ="${aws_eks_cluster.cluster.name}-managed_launch_template-blue-ondemand"    
    ...
    user_data = base64encode(local.worker-managed-node-userdata-blue-ondemand)
} 

resource "aws_eks_node_group" "workers_mng_blue" {
    count = var.activate-blue-asg-managed-ondemand == "true" ? 1 : 0
    node_group_name = "${aws_eks_cluster.cluster.name}-managed-node-blue-ondemand"
    ...
    launch_template {
        id =  aws_launch_template.managed_launch_template-blue-ondemand.id
        version = aws_launch_template.managed_launch_template-blue-ondemand.default_version
    } 
