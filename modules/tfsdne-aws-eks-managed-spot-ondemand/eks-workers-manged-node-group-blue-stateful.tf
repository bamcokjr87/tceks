locals {
  worker-managed-node-userdata-stateful_blue-spot = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_template" "managed_launch_template-stateful_blue" {  
  name     = "${aws_eks_cluster.cluster.name}-managed_launch_template-stateful_blue"
 ...
 # instance_type          = var.node_instance_type
  ...
  user_data              = base64encode(local.worker-managed-node-userdata-stateful_blue-spot)
}


resource "aws_eks_node_group" "workers_mng_stateful_blue_spot" {
  for_each        = var.activate-blue-asg-spot-managed-stateful == "true" ? {for i, subnet in var.subnet_ids: "AZ${i}"=> subnet} : {}
  node_group_name = "${aws_eks_cluster.cluster.name}-managed-node-stateful_blue-${each.key}" 
  ...
  launch_template {
    id      = aws_launch_template.managed_launch_template-stateful_blue.id
    version = aws_launch_template.managed_launch_template-stateful_blue.default_version
  }
