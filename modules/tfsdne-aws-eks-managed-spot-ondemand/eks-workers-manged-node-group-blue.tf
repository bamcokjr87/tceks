locals {
  worker-managed-node-userdata-blue-spot = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_template" "managed_launch_template-blue" {

  name     = "${aws_eks_cluster.cluster.name}-managed_launch_template-blue"
 ...
 # instance_type          = var.node_instance_type
  ...
  user_data              = base64encode(local.worker-managed-node-userdata-blue-spot)
}

resource "aws_eks_node_group" "workers_mng_blue_spot" {
  count           = var.activate-blue-asg-spot-managed == "true" ? 1 : 0
  node_group_name = "${aws_eks_cluster.cluster.name}-managed-node-blue" 
  ...
  launch_template {
    id      = aws_launch_template.managed_launch_template-blue.id
    version = aws_launch_template.managed_launch_template-blue.default_version
  }
