locals {
  worker-managed-node-userdata-green-ondemand = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_template" "managed_launch_template-green-ondemand"{
    
    name ="${aws_eks_cluster.cluster.name}-managed_launch_template-green-ondemand"    
    ...
    user_data = base64encode(local.worker-managed-node-userdata-green-ondemand)
} 

resource "aws_eks_node_group" "workers_mng_green" {
    count = var.activate-green-asg-managed-ondemand == "true" ? 1 : 0
    node_group_name = "${aws_eks_cluster.cluster.name}-managed-node-green-ondemand"
    ...
    launch_template {
        id =  aws_launch_template.managed_launch_template-green-ondemand.id
        version = aws_launch_template.managed_launch_template-green-ondemand.default_version
    } 
