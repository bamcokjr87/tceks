...
locals {
  worker-node-userdata-blue = <<USERDATA
#!/bin/bash
set -o xtrace
...
resource "aws_launch_configuration" "launch_configuration-blue" {
  ...
  user_data_base64 = base64encode(local.worker-node-userdata-blue)
...
resource "aws_autoscaling_group" "auto_scaling_group-blue" {

  count = var.activate-blue-asg == "true" ? 1 : 0

  desired_capacity     = var.auto_scaling_desired_size
  launch_configuration = aws_launch_configuration.launch_configuration-blue.id
